...
import naitokikaku.sscoordinator.domain.model.event.criteria.LikeEventName
...
class SearchEventSpec extends Specification {
    ...
    def "イベントを検索できる"() {
        setup:
        def event = new Event(new EventId(), new EventName("test event"))
        registerEvent.execute(event)
        def criteria = new EventCriteria(new LikeEventName("test"), new PaginationRequest())
